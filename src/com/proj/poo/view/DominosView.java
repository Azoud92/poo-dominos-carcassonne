...
import com.proj.poo.model.dominos.State;
	...
	public class Controle extends JPanel{
				...
				if (partie.getPlateau()[0][0]==null) {
					...
					while(partie.getPlateau()[place_y][place_x]!=null) {
						...
						if (place_y+1 == partie.getPlateau().length) {
							...
							if (winner!=null) {
								communication.setText(winner.pseudo + " a gagné la partie avec "+winner.getPoints()+" points.");
		...
		public void tour (){
			...
			if(!b) {
				...
				abandonBtn.setEnabled(false);
				if (winner!=null) {
					communication.setText("Bravo à "+ winner.pseudo + " qui a gagné la partie avec "+winner.getPoints()+" points.");
					info.add(communication);
					info.paintComponents(info.getGraphics());
				}
				else{
