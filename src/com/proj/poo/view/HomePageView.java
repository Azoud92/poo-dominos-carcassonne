...
public class HomePageView extends JPanel {
	...
	// cr√©ation de plusieurs panels afin de pouvoir utiliser les differents layout et bien placer les boutons
	public HomePageView(Dimension size, double scaleX, double scaleY, double scaleXY, JFrame frame, HomePageController hpc, Window window) {
		...
		container_1 = new JPanel(); // contient uniq. le bouton du domino (pour pouvoir les placer comme souhait√©)
		...
		title = new JLabel("Jeux de Dominos CarrÈs et de Carcassonne");
		title.setFont(new Font("Arial", Font.BOLD, (int) (60 * scaleX)));

		// contiendra le texte entr√© par l'utilisateur lorsque celui-ci ajoutera un joueur (qu'il soit IA ou R√©el) et qu'il mettra son pseudo
		players = new JPanel();
		...
		exitBtn.addActionListener(e -> {
			// action √† effectuer pour fermer le programme
			frame.dispose();
		});
		...
		// ajout de tous les container √† ceux correspondant
		...
		// requis pour que le fond d'√©cran soit visible
	...
	private class AjoutPlayer extends JPanel{
		...
		public AjoutPlayer() {
			...
			/* nouveau panel de 4 lignes contenant la requ√™te de pseudo, sa r√©ponse,
			 * le champ pour conna√Ætre le type de joueur (IA / R√©el), et le dernier pour valider
			 */
			...
			if (NB_PLAYERS_D > 0 || NB_PLAYERS_C > 0 ) { // on peut creer un bot seulement apres avoir crÔøΩÔøΩ au moins un vrai joueur
			...
			choix_partie.addActionListener(new ActionListener() {
	            public void actionPerformed(ActionEvent event) {
	            	...
	            	else if(choix_partie.getSelectedItem().equals("Dominos CarrÈs") && t.length>1){choix_color.setEnabled(false);}
			...
			valider.addActionListener((ActionEvent e) ->{ // d√®s qu'on clique sur le bouton valider
				if (!pseudo.getText().isEmpty() && !pseudo.getText().isBlank()) {// si le pseudo est non vide
					...
					if (!isBot.isSelected()) { // si c'est pas un bot
						if (choix_partie.getSelectedItem().equals("Dominos CarrÈs")) {
							NB_PLAYERS_D++;
							...
							if (NB_PLAYERS_C == 1) game.remove("Dominos CarrÈs");
						}
					}
					else { 
						if (choix_partie.getSelectedItem().equals("Dominos CarrÈs") && NB_PLAYERS_D != 0) {
							NB_PLAYERS_D++;
